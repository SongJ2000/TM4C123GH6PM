...
int OLED_Protocal = Protocal_IIC;
...
void OLED_Write_IIC_Command(unsigned char IIC_Command) {
  myIICWriteLen(SSD1306_I2C_ADDRESS, 0x00, 1, &IIC_Command);
}
/**********************************************
// IIC Write Data
**********************************************/
void OLED_Write_IIC_Data(unsigned char IIC_Data) {
  myIICWriteLen(SSD1306_I2C_ADDRESS, 0x40, 1, &IIC_Data);
}
void OLED_WR_Byte(unsigned char dat, unsigned char cmd) {
  switch (OLED_Protocal) {
    case Protocal_IIC:
      if (cmd) {
...
void fill_picture(unsigned char *fill_Data) {
  ...
  for (char m = 0; m < 8; m++) {
    myIICWriteLen(SSD1306_I2C_ADDRESS, 0x40, 128, fill_Data + 128 * m);
  }
}
